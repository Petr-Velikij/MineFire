execute if score @s PKM_2 matches 1.. run function minecraft:gun/splitting/shot
...
execute if score @s reload matches 0 unless score @s Supply matches 0 run function minecraft:gun/splitting/supply
execute unless score @s sneak_time matches 0 if score @s Supply matches 1.. run scoreboard players set @s reload 0
execute unless score @s reload matches 0 if score @s[nbt=!{SelectedItem:{tag:{Damage:0}}}] ammo matches 1.. run function minecraft:gun/splitting/reload_time