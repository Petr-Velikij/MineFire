...
execute positioned ~ ~1.1 ~ run summon minecraft:armor_stand ^-0.3 ^ ^0.2 {Silent:1b,Invulnerable:1b,Small:1b,Marker:1b,Invisible:1b,NoBasePlate:1b,PersistenceRequired:1b,Tags:["casing"],ArmorItems:[{},{},{},{id:"minecraft:carrot_on_a_stick",Count:1b,tag:{CustomModelData:900}}]}
scoreboard players set *1 varTime 900
scoreboard players operation *1 varTime += Inventory ID.gun
execute store result entity @e[type=armor_stand,tag=casing,sort=nearest,limit=1] ArmorItems[3].tag.CustomModelData int 1 run scoreboard players get *1 varTime
