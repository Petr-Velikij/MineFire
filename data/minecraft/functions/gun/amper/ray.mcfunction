...
execute if entity @e[distance=..5,tag=!NOtarget] unless entity @e[type=arrow,tag=my_amper,distance=..7] run function minecraft:gun/amper/spawn_arrow
execute positioned ~-0.5 ~-2.6 ~-0.5 if entity @e[gamemode=!spectator,dx=1,dz=1,dy=2,tag=!NOtarget,tag=!my_amper] run scoreboard players set *1 varTime 1
...
execute if score *2 varTime matches 0 if score @s ID.module matches 1 run particle dust 1 0.4 0 1 ~ ~ ~ 0 0 0 0 1 force @a[distance=..250,tag=!Owner]
execute if score *2 varTime matches 2 unless score @s ID.module matches 1 if score @s modeFire matches 2 run particle dust 1 0.4 0 2 ~ ~ ~ 0 0 0 0 1 force @a[distance=..250,tag=!Owner]
