...
summon armor_stand ~ ~1.6 ~ {NoGravity:1b,Silent:1b,Invulnerable:1b,Small:1b,Marker:1b,Invisible:1b,Tags:["knife"],Pose:{RightArm:[0f,-90f,-10f]},HandItems:[{id:"minecraft:iron_sword",Count:1b},{}]}
tp @e[type=armor_stand,tag=knife,limit=1,distance=..5,sort=nearest] ~ ~1.6 ~ ~90 0
function minecraft:gun/get_uuid
scoreboard players operation buf ID.Owner = @s ID.Player
playsound entity.player.attack.sweep player @a[distance=..20] ~ ~1.6 ~ 1 1

scoreboard players set *YR.correct varTime 8500
