...
execute if entity @s[nbt={OnGround:0b},scores={sneak_time=0}] unless data entity @s SelectedItem positioned ~ ~1.6 ~ positioned ^ ^-0.4 ^1 if block ~ ~ ~ #no_collider unless block ~ ~-0.5 ~ #no_collider run scoreboard players set *1 varTime 1
execute if score *1 varTime matches 1 if score @s armor_cooldown2 matches 0 positioned ~ ~1.6 ~ positioned ^ ^ ^0.8 run playsound minecraft:custom.armor.spetsnaz.parkour player @a[distance=..10] ~ ~ ~ 1 1
execute if score *1 varTime matches 1 run scoreboard players set @s armor_cooldown2 10
execute if score *1 varTime matches 1 run effect give @s levitation 1 24 true
